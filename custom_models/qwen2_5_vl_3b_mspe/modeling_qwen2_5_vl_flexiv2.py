...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_window_patchsize(
            self,
            grid_thw: torch.Tensor,  # shape [n, 3], each row: [T, H, W]
            window_size: int = 8
    ) -> Tuple[List[int], List[torch.Tensor]]:
        """
        Args:
            grid_thw: torch.Tensor, [n, 3], each row is [T, H, W]
            window_size: int, default 8
        Returns:
            window_patchsize_list: List[int], patch size for each window
            window_grid_thw_list: List[torch.Tensor], each tensor [T, h_win, w_win] per window
        """
        window_patchsize_list = []
        window_grid_thw_list = []
        ...
        for t, h, w in grid_thw.tolist():
            ...
            for hi in range(num_h):
                for wi in range(num_w):
                    h_win = min(window_size, h - hi * window_size)
                    w_win = min(window_size, w - wi * window_size)
                    window_grid_thw_list.append(torch.tensor([t, h_win, w_win]))
                    # Select patch size
                    if h_win == window_size and w_win == window_size:
                        patchsize = random.choice([7, 14, 28])
                    elif h_win % 4 == 0 and w_win % 4 == 0:
                        patchsize = random.choice([7, 14, 28])
                    else:
                        patchsize = random.choice([7, 14])
                    # window_patchsize_list.append(patchsize)
                    window_patchsize_list.append(14)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> Tuple[torch.Tensor, List]:
        ...
        hidden_states_ps7 = self.patch_embed(hidden_states)
        hidden_states_ps14 = self.patch_embed(hidden_states)
        hidden_states_ps28 = self.patch_embed(hidden_states)
        for i, (window_idx, window_patchsize, window_grid_thw) in enumerate(
            ...
            hidden_states_list.append(window_hidden_states)
