...
class Qwen2VLImageProcessorMSPE(BaseImageProcessor):
    ...
    def preprocess(
        ...
        if images is not None:
            ...
            # TODO：for MSPE
            pixel_values_dict, vision_grid_thws_dict = {}, {}
            for ps in [7, 14, 28]:
                pixel_values, vision_grid_thws = [], []
                for image in images:
                    patches, image_grid_thw = self._preprocess(
                        ...
                        image_mean=image_mean,
                        image_std=image_std,
                        patch_size=ps,  # ✅ loop over patch size
                        temporal_patch_size=temporal_patch_size,
                        merge_size=merge_size,
                        data_format=data_format,
                        do_convert_rgb=do_convert_rgb,
                ...
                pixel_values_dict[f"pixel_values_ps{ps}"] = pixel_values
                vision_grid_thws_dict[f"image_grid_thw_ps{ps}"] = vision_grid_thws

            # ✅ update data with all patch sizes
            data.update(pixel_values_dict)
            data.update(vision_grid_thws_dict)
